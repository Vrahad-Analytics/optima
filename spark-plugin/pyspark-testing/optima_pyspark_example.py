#!/usr/bin/python3
"""
PySpark example with Optima instrumentation for all Python exec types:
  - mapInPandas         → OptimaMapInPandasExec
  - mapInArrow          → OptimaPythonMapInArrowExec
  - Window (SQL)        → OptimaWindowExec
  - Window (pandas UDF) → OptimaWindowInPandasExec
  - pandas_udf SCALAR   → OptimaArrowEvalPythonExec
  - applyInPandas       → OptimaFlatMapGroupsInPandasExec
  - cogroup             → OptimaFlatMapCoGroupsInPandasExec

Usage:
    pip install pyspark pandas pyarrow
    python optima_pyspark_example.py
"""
...
spark = SparkSession \
    .builder \
    .appName("Optima Pyspark Example") \
    .config("spark.jars", str(_plugin_jar)) \
    .config("spark.plugins", "io.telemetria.optima.SparkOptimaPlugin") \
    .config("spark.ui.port", "10000") \
    .config("spark.sql.maxMetadataStringLength", "10000") \
    .config("spark.sql.adaptive.enabled", "true") \
    .config("spark.optima.instrument.spark.mapInPandas.enabled", instrument) \
    .config("spark.optima.instrument.spark.mapInArrow.enabled", instrument) \
    .config("spark.optima.instrument.spark.window.enabled", instrument) \
    .config("spark.optima.instrument.spark.arrowEvalPython.enabled", instrument) \
    .config("spark.optima.instrument.spark.batchEvalPython.enabled", instrument) \
    .config("spark.optima.instrument.spark.flatMapGroupsInPandas.enabled", instrument) \
    .config("spark.optima.instrument.spark.flatMapCoGroupsInPandas.enabled", instrument) \
    .config("spark.optima.instrument.spark.sqlNodes.enabled", instrument) \
    .master("local[*]") \
    .getOrCreate()
# .config("spark.optima.test.codegenSleepMs", "1") \
#     .config("spark.sql.codegen.wholeStage", "false") \
# spark.sparkContext.setLogLevel("INFO")

# Register the SlowSumAggregator (Scala UDAF) via py4j after the SparkSession exists
_jvm = spark._jvm
_aggregator = _jvm.org.apache.spark.optima.SlowSumAggregator()
...
spark.sparkContext.setJobDescription("mapInPandas: compute discounted totals → OptimaMapInPandasExec")
df_pandas.write \
    .mode("overwrite") \
    .parquet("/tmp/optima_map_in_pandas_example")

print("\nResult written to /tmp/optima_map_in_pandas_example")
...
if supports_map_in_arrow:
    ...
    spark.sparkContext.setJobDescription("mapInArrow: compute discounted totals → OptimaPythonMapInArrowExec")
    df_arrow.write \
        .mode("overwrite") \
        .parquet("/tmp/optima_map_in_arrow_example")

    print("\nResult written to /tmp/optima_map_in_arrow_example")
else:
    print("\n" + "="*80)
...
# slow_sum is a Scala UDAF registered by OptimaInstrumentationExtension.
# It sums Doubles but sleeps `spark.optima.test.slowSumSleepMs` per row on the JVM.
...
spark.sparkContext.setJobDescription("Window SQL: rank + slow_sum (Scala UDAF) + avg → OptimaWindowExec")
df_window.write \
    .mode("overwrite") \
    .parquet("/tmp/optima_window_example")

print("\nResult written to /tmp/optima_window_example")
...
spark.sparkContext.setJobDescription("Window pandas UDF: discounted sum → OptimaWindowInPandasExec")
df_window_udf.write \
    .mode("overwrite") \
    .parquet("/tmp/optima_window_udf_example")

print("\nResult written to /tmp/optima_window_udf_example")
...
# Regular Python @udf → OptimaBatchEvalPythonExec
...
spark.sparkContext.setJobDescription("@udf: classify price tier → OptimaBatchEvalPythonExec")
df_batch_udf.write \
    .mode("overwrite") \
    .parquet("/tmp/optima_batch_eval_python_example")

print("\nResult written to /tmp/optima_batch_eval_python_example")
...
spark.sparkContext.setJobDescription("pandas_udf SCALAR: apply discount → OptimaArrowEvalPythonExec")
df_scalar_udf.write \
    .mode("overwrite") \
    .parquet("/tmp/optima_arrow_eval_python_example")

print("\nResult written to /tmp/optima_arrow_eval_python_example")
...
# applyInPandas / GROUPED_MAP → OptimaFlatMapGroupsInPandasExec
...
spark.sparkContext.setJobDescription("applyInPandas: per-category stats → OptimaFlatMapGroupsInPandasExec")
df_grouped.write \
    .mode("overwrite") \
    .parquet("/tmp/optima_flat_map_groups_in_pandas_example")

print("\nResult written to /tmp/optima_flat_map_groups_in_pandas_example")
...
# cogroup / applyInPandas on two DataFrames → OptimaFlatMapCoGroupsInPandasExec
...
spark.sparkContext.setJobDescription("cogroup applyInPandas: apply category discounts → OptimaFlatMapCoGroupsInPandasExec")
df_cogrouped.write \
    .mode("overwrite") \
    .parquet("/tmp/optima_flat_map_cogroups_in_pandas_example")

print("\nResult written to /tmp/optima_flat_map_cogroups_in_pandas_example")
...
df.write.mode("overwrite").parquet("/tmp/optima_columnar_test_input")
df_parquet = spark.read.parquet("/tmp/optima_columnar_test_input")
...
df_parquet.withColumn("doubled_price", double_price("price")) \
    ...
    .write.mode("overwrite").parquet("/tmp/optima_columnar_udf_shuffle_example")
print("\nResult written to /tmp/optima_columnar_udf_shuffle_example")
...
df.filter(col("price") > 200) \
  ...
  .write.mode("overwrite").parquet("/tmp/optima_filter_project_example")
print("\nResult written to /tmp/optima_filter_project_example")
...
df.rollup("category", "customer") \
  ...
  .write.mode("overwrite").parquet("/tmp/optima_expand_example")
print("\nResult written to /tmp/optima_expand_example")
...
df.select("customer", "category",
  ...
  .write.mode("overwrite").parquet("/tmp/optima_generate_example")
print("\nResult written to /tmp/optima_generate_example")
...
df.join(category_tiers, "category") \
  .write.mode("overwrite").parquet("/tmp/optima_broadcast_hash_join_example")
print("\nResult written to /tmp/optima_broadcast_hash_join_example")
...
#   .write.mode("overwrite").parquet("/tmp/optima_sort_merge_join_example")
# spark.conf.unset("spark.sql.autoBroadcastJoinThreshold")
# print("\nResult written to /tmp/optima_sort_merge_join_example")
...
df.join(price_ranges, (col("price") >= col("min_p")) & (col("price") <= col("max_p"))) \
  ...
  .write.mode("overwrite").parquet("/tmp/optima_bnlj_example")
print("\nResult written to /tmp/optima_bnlj_example")
...
df.crossJoin(currencies) \
  ...
  .write.mode("overwrite").parquet("/tmp/optima_cartesian_example")
spark.conf.unset("spark.sql.autoBroadcastJoinThreshold")
print("\nResult written to /tmp/optima_cartesian_example")
...
df.withColumn("rn", row_number().over(w_top)) \
  .filter(col("rn") <= 3) \
  .drop("rn") \
  .write.mode("overwrite").parquet("/tmp/optima_window_group_limit_example")
print("\nResult written to /tmp/optima_window_group_limit_example")
...
df.groupBy("category") \
  ...
  .write.mode("overwrite").parquet("/tmp/optima_sort_agg_example")
spark.conf.set("spark.sql.execution.useObjectHashAggregateExec", "true")
print("\nResult written to /tmp/optima_sort_agg_example")
...
high_value.union(low_value) \
    .write.mode("overwrite").parquet("/tmp/optima_cache_filter_union_example")

cached_df.unpersist()
print("\nResult written to /tmp/optima_cache_filter_union_example")
...
df_complex.select(
  ...
  .write.mode("overwrite").parquet("/tmp/optima_rddscan_complex_codegen_test")
print("\nResult written to /tmp/optima_rddscan_complex_codegen_test")
...
df.select(
  ...
  .write.mode("overwrite").parquet("/tmp/optima_rddscan_codegen_test")
print("\nResult written to /tmp/optima_rddscan_codegen_test")
