 ...
 */
class OptimaSqlNodesSpec extends AnyFunSuite with Matchers with BeforeAndAfterAll {
  ...
  override def beforeAll(): Unit = {
    tmpDir = Files.createTempDirectory("optima_test").toString
    spark = SparkSession.builder()
      .master("local[1]")
      .appName("OptimaSqlNodesSpec")
      .config(OptimaSparkUICommonLoader.INSTRUMENT_SQL_NODES_ENABLED, "true")
      .config("spark.sql.adaptive.enabled", "false")
      .config("spark.ui.enabled", "false")
      .config("spark.optima.test.codegenSleepMs", sleepInCodeGenMs)
