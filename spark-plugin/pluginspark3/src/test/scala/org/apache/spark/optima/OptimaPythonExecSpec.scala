...
class OptimaPythonExecSpec extends AnyFunSuite with Matchers with BeforeAndAfterAll {
  ...
  test("wraps MapInPandasExec with TimedExec") {
    ...
    val rule = OptimaInstrumentationColumnarRule(spark)
