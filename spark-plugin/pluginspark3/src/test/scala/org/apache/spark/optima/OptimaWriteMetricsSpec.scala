 ...
 */
class OptimaWriteMetricsSpec extends AnyFunSuite with Matchers with BeforeAndAfterAll {
  ...
  override def beforeAll(): Unit = {
    spark = SparkSession.builder()
      .master("local[1]")
      .appName("OptimaWriteMetricsSpec")
      .config(OptimaSparkUICommonLoader.INSTRUMENT_SQL_NODES_ENABLED, "true")
      .config("spark.sql.adaptive.enabled", "false")
      .config("spark.ui.enabled", "false")
      .withExtensions(new OptimaInstrumentationExtension)
      .getOrCreate()
  ...
  test("DataWritingCommandExec metrics survive the executeCollect rebuild") {
    val tempDir = java.nio.file.Files.createTempDirectory("optima-write-test").toFile
    try {
      capturedQE.set(null)
      val session = spark
      ...
      // The Optima-wrapped DataWritingCommandExec — metrics on this node feed the SparkUI.
