...
class OptimaWindowExecSpec extends AnyFunSuite with Matchers with BeforeAndAfterAll with SqlMetricTestHelper {
  ...
  override def beforeAll(): Unit = {
    spark = SparkSession.builder()
      .master("local[1]")
      .appName("OptimaWindowExecSpec")
      .config(OptimaSparkUICommonLoader.INSTRUMENT_SPARK_ENABLED, "true")
      .config(OptimaSparkUICommonLoader.INSTRUMENT_WINDOW_ENABLED, "true")
  ...
  test("OptimaInstrumentationColumnarRule wraps WindowExec with TimedExec") {
    val session = spark
