...
/**
 * A SparkSessionExtensions that injects Optima instrumentation into Spark's physical planning phase.
 */
class OptimaInstrumentationExtension extends (SparkSessionExtensions => Unit) with Logging {

  override def apply(extensions: SparkSessionExtensions): Unit = {
    logInfo("Registering Optima Instrumentation Extension")

    extensions.injectColumnar { session =>
      OptimaInstrumentationColumnarRule(session)
    }
  }
}
 ...
 * }}}
 *
 * Since the Optima plugin's `init()` runs first (it auto-registers this extension before
 * the accelerator's plugin appends its own), Optima sits at index 0 in `columnarRules` and
 ...
 */
case class OptimaInstrumentationColumnarRule(session: SparkSession) extends ColumnarRule with Logging {
  ...
  private val enabledNodeNames: Set[String] = {
    ...
    val globalEnabled = conf.getBoolean(OptimaSparkUICommonLoader.INSTRUMENT_SPARK_ENABLED, defaultValue = false)
    val sqlNodes = Set(
    ...
    if (globalEnabled) all
      ...
      (if (conf.getBoolean(OptimaSparkUICommonLoader.INSTRUMENT_SQL_NODES_ENABLED, false))         sqlNodes                                          else Set.empty[String]) ++
      (if (conf.getBoolean(OptimaSparkUICommonLoader.INSTRUMENT_BATCH_EVAL_PYTHON_ENABLED, false))     Set("BatchEvalPythonExec")                    else Set.empty[String]) ++
      (if (conf.getBoolean(OptimaSparkUICommonLoader.INSTRUMENT_ARROW_EVAL_PYTHON_ENABLED, false))     Set("ArrowEvalPythonExec")                    else Set.empty[String]) ++
      (if (conf.getBoolean(OptimaSparkUICommonLoader.INSTRUMENT_MAP_IN_PANDAS_ENABLED, false))         Set("MapInPandasExec")                        else Set.empty[String]) ++
      (if (conf.getBoolean(OptimaSparkUICommonLoader.INSTRUMENT_MAP_IN_ARROW_ENABLED, false))          Set("PythonMapInArrowExec")                   else Set.empty[String]) ++
      (if (conf.getBoolean(OptimaSparkUICommonLoader.INSTRUMENT_FLAT_MAP_GROUPS_PANDAS_ENABLED, false)) Set("FlatMapGroupsInPandasExec")             else Set.empty[String]) ++
      (if (conf.getBoolean(OptimaSparkUICommonLoader.INSTRUMENT_FLAT_MAP_COGROUPS_PANDAS_ENABLED, false)) Set("FlatMapCoGroupsInPandasExec")         else Set.empty[String]) ++
      (if (conf.getBoolean(OptimaSparkUICommonLoader.INSTRUMENT_WINDOW_ENABLED, false))                Set("WindowExec", "WindowInPandasExec")       else Set.empty[String])
    }
  }
  ...
  if (enabledNodeNames.nonEmpty) {
    ...
    if (active.nonEmpty) {
      logInfo(
        s"Optima: native accelerator(s) detected — ${active.map(_.name).mkString(", ")}. " +
          "Optima runs in postColumnarTransitions and only wraps operators no accelerator transformed.")
    }
  }
  ...
  override def postColumnarTransitions: Rule[SparkPlan] = { plan =>
    if (enabledNodeNames.isEmpty) plan
      ...
      case node if enabledNodeNames.contains(node.getClass.getSimpleName)
        ...
        logInfo(s"Optima: wrapping ${node.getClass.getSimpleName} with TimedExec")
        TimedExec(node)
    }
  }
}
