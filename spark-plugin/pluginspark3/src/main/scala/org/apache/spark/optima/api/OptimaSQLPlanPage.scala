...
class OptimaSQLPlanPage(ui: SparkUI, optimaStore: OptimaStore, sqlListener: () => Option[SQLAppStatusListener])
  ...
  override def renderJson(request: HttpServletRequest) = {
    try {
      ...
      if (offset == null || length == null) {
        ...
        val nodeIdToRddScopeIdList = optimaStore.databricksAdditionalExecutionInfo(offset.toInt, length.toInt)
      ...
      case e: Throwable => {
        logError("failed to serve optima SQL metrics", e)
        JArray(List())
      }
    }
  }
