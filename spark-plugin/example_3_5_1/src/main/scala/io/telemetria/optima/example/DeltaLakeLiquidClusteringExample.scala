...
object DeltaLakeLiquidClusteringExample extends App {
  val spark = SparkSession
    ...
    .config("spark.optima.instrument.deltalake.enabled", true)
    // Optional: Configure z-index field collection and caching behavior
     .config("spark.optima.instrument.deltalake.collectZindexFields", true) // default: true
    // .config("spark.optima.instrument.deltalake.cacheZindexFieldsToProperties", true) // default: true
  ...
  ZORDER BY (category)
""")
  println("Z-Order optimization completed - Optima will detect and cache z-order fields")
