...
import org.apache.spark.optima.listener.{OptimaExecutorStorageInfo, OptimaRDDStorageInfo, OptimaRDDStorageInfoWrapper}
...
private[spark] class LiveRDDsListener(store: ElementTrackingStore) extends SparkListener with Logging {
  ...
  override def onUnpersistRDD(event: SparkListenerUnpersistRDD): Unit = {
    liveRDDs.get(event.rddId).foreach { rdd => {
      ...
      val maxExecutorInfo = maxExecutorData.map(executor => OptimaExecutorStorageInfo(executor.memoryUsed, executor.memoryRemaining, executor.memoryUsed.toDouble / (executor.memoryUsed + executor.memoryRemaining) * 100))
      store.write(new OptimaRDDStorageInfoWrapper(OptimaRDDStorageInfo(rdd.info.id, rdd.memoryUsed, rdd.diskUsed, rdd.info.numPartitions, rdd.info.storageLevel.description, maxExecutorInfo)))
    }}
