...
object OptimaDatabricksLiveListener {
  def apply(listenerBus: LiveListenerBus): OptimaDatabricksLiveListener = {
    val rddScopeIdReader = classOf[SparkPlanGraphNode].getMethod("rddScopeId")
    new OptimaDatabricksLiveListener(listenerBus, rddScopeIdReader)
  }
}

class OptimaDatabricksLiveListener(listenerBus: LiveListenerBus, rddScopeIdReader: Method) extends SparkListener with Logging {
  ...
  override def onOtherEvent(event: SparkListenerEvent): Unit = {
    try {
      ...
      case e: Throwable => logError("Error while processing events in OptimaDatabricksLiveListener", e)
    }
  }
}
