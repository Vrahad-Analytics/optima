...
class DeltaLakeInstrumentationListener(
    ...
    sparkSession match {
      case Some(spark) =>
        try {
          ...
          val scanInfo = OptimaDeltaLakeScanInfo(
          ...
          val event = OptimaDeltaLakeScanEvent(scanInfo)
              ...
              reflectionUtils.callMetadata(snapshot) match {
                case Some(metadata) =>
                  ...
                  val cachedZorderColumns = reflectionUtils.callConfiguration(metadata).flatMap { customProperties =>
                    ...
                    customProperties.get("optima.zorderFields").map { cols =>
  ...
  private def getZOrderColumns(spark: SparkSession, tableNameOrPath: String, rootExecutionId: Long): Seq[String] = {
    Try {
      ...
      spark.sparkContext.setJobDescription(s"Optima Delta Lake Instrumentation - search operations history to find Z-Order fields for table $tableNameOrPath")
      ...
      if (cacheZindexFieldsToProperties) {
        Try {
          ...
          if (zOrderColumns.nonEmpty) {
            logInfo(s"Set optima.zorderFields metadata for table $tableNameOrPath: $metadataValue")
            spark.sparkContext.setJobDescription(s"Optima Delta Lake Instrumentation - add found z-order fields to table properties for table $tableNameOrPath")
          } else {
            logInfo(s"Set optima.zorderFields metadata to empty for table $tableNameOrPath (no z-order fields found)")
            spark.sparkContext.setJobDescription(s"Optima Delta Lake Instrumentation - add no z-order fields to table properties for table $tableNameOrPath")
          }
          ...
          val alterTableCmd = if (tableNameOrPath.contains("/") || tableNameOrPath.contains(":\\") || tableNameOrPath.contains("s3://") || tableNameOrPath.contains("dbfs:/")) {
            ...
            s"ALTER TABLE delta.`$tableNameOrPath` SET TBLPROPERTIES ('optima.zorderFields' = '$metadataValue')"
          } else {
            // It's a qualified table name, use it directly
            s"ALTER TABLE $tableNameOrPath SET TBLPROPERTIES ('optima.zorderFields' = '$metadataValue')"
          }
