 ...
 * SQL UI and the Optima UI.
 *
 * ## Node name
 * `nodeName` is prefixed with "Optima" so the node is clearly identified in plan displays.
 *
...
class TimedExec(val child: SparkPlan) extends SparkPlan with Logging {
  override def nodeName: String = "Optima" + child.nodeName
  ...
  override protected def doExecute(): RDD[InternalRow] = {
    ...
    OptimaRDDUtils.withDurationMetric(childRdd, longMetric("duration"))
  }

  override protected def doExecuteColumnar(): RDD[ColumnarBatch] = {
    val childRdd = child.executeColumnar()
    postRddId(childRdd.id)
    OptimaRDDUtils.withDurationMetricColumnar(childRdd, longMetric("duration"))
  }
...
class TimedWithCodegenExec(override val child: SparkPlan) extends TimedExec(child) with CodegenSupport {
  override def nodeName: String = ("Optima" + child.nodeName).replace(" ", "")
  ...
  override protected def doProduce(ctx: CodegenContext): String = {
    ...
    val sleepMs = sparkContext.conf.getLong("spark.optima.test.codegenSleepMs", 0L)
...
object TimedExec {
   ...
   */
  private[optima] class RDDTimingWrapper(val child: SparkPlan, durationMetric: SQLMetric) extends SparkPlan {
    override def output: Seq[Attribute] = child.output
    ...
    override protected def doExecute(): RDD[InternalRow] =
      OptimaRDDUtils.withDurationMetric(child.execute(), durationMetric)

    override protected def doExecuteColumnar(): RDD[ColumnarBatch] =
      OptimaRDDUtils.withDurationMetricColumnar(child.executeColumnar(), durationMetric)
