...
case class OptimaEnvironmentInfo(driverXmxBytes: Long)

case class OptimaEnvironmentInfoEvent(environmentInfo: OptimaEnvironmentInfo) extends SparkListenerEvent

class OptimaEnvironmentInfoWrapper(val info: OptimaEnvironmentInfo) {
  @KVIndex
  @JsonIgnore
  def id: String = "environment_info"
}

case class OptimaExecutorStorageInfo(memoryUsed: Long, memoryRemaining: Long, memoryUsagePercentage: Double)

case class OptimaRDDStorageInfo(rddId: Int, memoryUsed: Long, diskUsed: Long, numOfPartitions: Int, storageLevel: String, maxMemoryExecutorInfo: Option[OptimaExecutorStorageInfo])

class OptimaRDDStorageInfoWrapper(val info: OptimaRDDStorageInfo) {
  @KVIndex
  def id: Int = info.rddId
}

case class OptimaDeltaLakeScanInfo(
                                       minExecutionId: Long,
                                       tablePath: String,
...
case class OptimaDeltaLakeScanEvent(scanInfo: OptimaDeltaLakeScanInfo) extends SparkListenerEvent

class OptimaDeltaLakeScanInfoWrapper(val info: OptimaDeltaLakeScanInfo) {
  @KVIndex
  @JsonIgnore
