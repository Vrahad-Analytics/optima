...
import io.telemetria.optima.SparkOptima
...
object IcebergExample333 extends App with Logging {
  ...
  val spark = SparkSession
    ...
    .config("spark.sql.catalog.local.metrics-reporter-impl", "org.apache.spark.optima.iceberg.OptimaIcebergMetricsReporter")

    .config("spark.sql.catalog.local.type", "hadoop")
    .config("spark.sql.catalog.local.warehouse", "/tmp/iceberg-example/warehouse")
    .config("spark.sql.defaultCatalog", "local")
    .config("spark.optima.iceberg.autoCatalogDiscovery", false)
    .config("spark.eventLog.enabled", "true")
    .master("local[*]")
    .getOrCreate()

  SparkOptima.install(spark.sparkContext)
